...
function DetailTableV2({ cases, prices, loadingIds, onSelect, onPriceChange }: { cases: BacktestCase[]; prices: Record<string, number>; loadingIds: Set<string>; onSelect: (testCase: BacktestCase) => void; onPriceChange: (id: string, value: number) => void }) {
  ...
  const columns: DetailColumn[] = [
    ...
    { key: "analysisPrice", label: "Harga Analisis", width: "w-[180px]", group: "IDENTITY", render: (item) => <EditablePriceCell value={valueFor(item)} isLoading={loadingIds.has(item.id)} onCommit={(value) => onPriceChange(item.id, value)} /> },
    ...
    { key: "peakReturn", label: "Ret Peak", width: "w-[110px]", group: "PEAK / TROUGH", render: (item) => percent(getDerivedData(item, valueFor(item)).peakReturn) },
    { key: "peakPrice", label: "Harga Peak", width: "w-[120px]", group: "PEAK / TROUGH", render: (item) => price(getDerivedData(item, valueFor(item)).peakPrice) },
    ...
    { key: "troughReturn", label: "Ret Trough", width: "w-[120px]", group: "PEAK / TROUGH", render: (item) => percent(getDerivedData(item, valueFor(item)).troughReturn) },
    { key: "downPrice", label: "Harga Trough", width: "w-[135px]", group: "PEAK / TROUGH", render: (item) => price(getDerivedData(item, valueFor(item)).downPrice) },
  ...
  return <div className="overflow-x-auto rounded-xl border border-white/8"><table className="w-max min-w-full border-collapse text-left text-xs"><thead className="bg-[#081523] text-[9px] uppercase tracking-wider text-[#7f8fa6]"><tr>{groups.map((group) => <th key={group.label} colSpan={group.span} className={`border-r-2 border-b-2 border-[#07111c] px-3 py-3 text-left align-top font-bold tracking-[0.12em] ${group.className}`}>{group.label}</th>)}</tr><tr>{columns.map((column, index) => <th key={column.key} className={`${column.width} whitespace-nowrap border-r border-white/10 px-3 py-3 text-left align-top ${column.key === "methods" ? "text-center" : ["analysisPrice", "mosMain", "mosPeter", "mosWeight", "yield", "revenueYoY", "revPercent", "netIncomeYoY", "niPercent", "ret3m", "ret6m", "ret9m", "ret12m", "price3m", "price6m", "price9m", "price12m", "peakReturn", "downPrice", "peakPrice", "troughReturn"].includes(column.key) ? "text-right" : ""} ${index === 0 || columns[index - 1]?.group !== column.group ? "border-l-2 border-l-[#07111c] bg-white/[0.04]" : ""}`}>{column.label}</th>)}</tr></thead><tbody>{cases.map((item) => <tr key={item.id} aria-busy={loadingIds.has(item.id)} className={`border-t border-white/8 transition ${loadingIds.has(item.id) ? "pointer-events-none bg-[#f2bb5c]/10 opacity-60" : "hover:bg-white/[0.02]"}`}>{columns.map((column, index) => <td key={column.key} className={`${column.width} whitespace-nowrap border-r border-white/8 px-3 py-3 text-[#b9c6d8] ${column.key === "methods" ? "text-center" : ["analysisPrice", "mosMain", "mosPeter", "mosWeight", "yield", "revenueYoY", "revPercent", "netIncomeYoY", "niPercent", "ret3m", "ret6m", "ret9m", "ret12m", "price3m", "price6m", "price9m", "price12m", "peakReturn", "downPrice", "peakPrice", "troughReturn"].includes(column.key) ? "text-right" : ""} ${index === 0 || columns[index - 1]?.group !== column.group ? "border-l-2 border-l-[#07111c] bg-white/[0.015]" : ""}`}>{column.render(item)}</td>)}</tr>)}</tbody></table></div>;
}
...
export function BacktestTabContent({ stock }: { stock: StockDetail }) {
  ...
  const summary = useMemo(() => {
    ...
    const methodUndervalued = cases.filter((item) => consensusByMethod(item, analysisPrices[item.id] ?? item.analysisPrice) === "UNDERVALUED").length;
    const mosUndervalued = cases.filter((item) => consensusByMos(item) === "UNDERVALUED").length;
    return {
      total,
      methodUndervalued,
      methodOvervalued: total - methodUndervalued,
      methodWins: cases.filter((item) => item.verdict === "WIN").length,
      mosUndervalued,
      mosOvervalued: total - mosUndervalued,
      mosWins: cases.filter((item) => item.verdictMos === "WIN").length,
    };
  }, [analysisPrices, cases]);
  return <div className="space-y-6"><header><h2 className="text-3xl font-bold tracking-tight text-white">Backtest</h2><p className="mt-1 text-sm text-[#aeb9ca]">See how the valuation framework would have performed across historical cases.</p></header><SectionCard title="Overview" subtitle="Summary of historical cases based on the valuation framework."><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-7"><SummaryCard label="Total Cases" value={String(summary.total)} context="Analyzed quarters" icon="list" /><SummaryCard label="Undervalue Cases" value={String(summary.methodUndervalued)} context="Verdict by Method" icon="up" tone="text-[#3ef0a9]" /><SummaryCard label="Overvalued Cases" value={String(summary.methodOvervalued)} context="Verdict by Method" icon="down" tone="text-[#75c5ff]" /><SummaryCard label="Verdict Win" value={String(summary.methodWins)} context="By Method" icon="up" tone="text-[#3ef0a9]" /><SummaryCard label="Undervalue Cases" value={String(summary.mosUndervalued)} context="Verdict by MoS" icon="up" tone="text-[#3ef0a9]" /><SummaryCard label="Overvalued Cases" value={String(summary.mosOvervalued)} context="Verdict by MoS" icon="down" tone="text-[#75c5ff]" /><SummaryCard label="Verdict Win" value={String(summary.mosWins)} context="By MoS" icon="up" tone="text-[#3ef0a9]" /></div></SectionCard><SectionCard title="Historical Cases" subtitle="List of historical valuation cases. Click a quarter to see the case detail." actionSlot={<div className="flex items-center gap-1 self-start rounded-xl border border-white/10 bg-[#091322]/80 p-1 text-xs"><button type="button" onClick={() => setView("compact")} className={`rounded-lg px-4 py-1.5 font-semibold transition ${view === "compact" ? "bg-[#f2bb5c] text-[#1d170f]" : "text-[#9aa9bf] hover:bg-white/5 hover:text-white"}`}>Compact View</button><button type="button" onClick={() => setView("detail")} className={`rounded-lg px-4 py-1.5 font-semibold transition ${view === "detail" ? "bg-[#f2bb5c] text-[#1d170f]" : "text-[#9aa9bf] hover:bg-white/5 hover:text-white"}`}>Detail View</button></div>}>{cases.length === 0 ? <div className="p-10 text-center text-sm text-[#9aa9bf]">No historical backtest cases are available for {stock.ticker} yet.</div> : view === "compact" ? <CompactTable cases={cases} prices={analysisPrices} onSelect={setSelectedCase} onPriceChange={updateAnalysisPrice} /> : <DetailTableV2 cases={cases} prices={analysisPrices} loadingIds={loadingPriceIds} onSelect={setSelectedCase} onPriceChange={updateAnalysisPrice} />}</SectionCard><SectionCard title="How the Backtest Works" subtitle="A simple and transparent process to evaluate historical outcomes."><div className="grid gap-4 lg:grid-cols-3"><Step number="01" title="Identify the Condition" text="Each historical quarter is classified as Undervalued, Overvalued, or Mixed using the valuation framework available on the analysis date." /><Step number="02" title="Track Price Movement" text="After the analysis date, price movement is observed for up to 12 months against predefined upside and downside thresholds." /><Step number="03" title="Determine the Outcome" text="The first threshold reached determines the historical outcome. This is evidence of past behavior, not a prediction or recommendation." /></div></SectionCard>{selectedCase && <DetailDrawer testCase={selectedCase} onClose={() => setSelectedCase(null)} />}</div>;
}
