...
const [horizon3M, horizon6M, horizon9M, horizon12M] = backtestMethodology.horizonsMonths;
...
function consensusByMethod(testCase: BacktestCase, analysisPrice = testCase.analysisPrice): BacktestConsensus {
  return classifyMethodsAbovePrice(testCase.methods, analysisPrice).classification === "UNDERVALUED" ? "UNDERVALUED" : "OVERVALUED";
}

function consensusByMos(testCase: BacktestCase): BacktestConsensus {
  return classifyMosMain(testCase.mosMain) === "UNDERVALUED" ? "UNDERVALUED" : "OVERVALUED";
}
...
function DetailTable({ cases, prices, onSelect, onPriceChange }: { cases: BacktestCase[]; prices: Record<string, number>; onSelect: (testCase: BacktestCase) => void; onPriceChange: (id: string, value: number) => void }) {
  const columns: DetailColumn[] = [
    ...
    { key: "ret3m", label: "Ret 3M", width: "w-[100px]", group: "BACKTEST", render: (item) => percent(calculateBacktestMetrics(item).returns[0]) }, { key: "ret6m", label: "Ret 6M", width: "w-[100px]", group: "BACKTEST", render: (item) => percent(calculateBacktestMetrics(item).returns[1]) }, { key: "ret9m", label: "Ret 9M", width: "w-[100px]", group: "BACKTEST", render: (item) => percent(calculateBacktestMetrics(item).returns[2]) }, { key: "ret12m", label: "Ret 12M", width: "w-[105px]", group: "BACKTEST", render: (item) => percent(calculateBacktestMetrics(item).returns[3]) }, { key: "price3m", label: "Price 3M", width: "w-[115px]", group: "BACKTEST", render: (item) => price(closeAtBacktestHorizon(item, horizon3M)) }, { key: "price6m", label: "Price 6M", width: "w-[115px]", group: "BACKTEST", render: (item) => price(closeAtBacktestHorizon(item, horizon6M)) }, { key: "price9m", label: "Price 9M", width: "w-[115px]", group: "BACKTEST", render: (item) => price(closeAtBacktestHorizon(item, horizon9M)) }, { key: "price12m", label: "Price 12M", width: "w-[120px]", group: "BACKTEST", render: (item) => price(closeAtBacktestHorizon(item, horizon12M)) }, { key: "peakReturn", label: "Ret Peak", width: "w-[110px]", group: "BACKTEST", render: (item) => percent(calculateBacktestMetrics(item).peakReturn) }, { key: "downPrice", label: "Harga Down", width: "w-[120px]", group: "BACKTEST", render: (item) => price(calculateBacktestMetrics(item).downPrice) }, { key: "peakPrice", label: "Harga Peak", width: "w-[120px]", group: "BACKTEST", render: (item) => price(calculateBacktestMetrics(item).peakPrice) }, { key: "peakMonth", label: "Bln Peak", width: "w-[100px]", group: "BACKTEST", render: (item) => { const month = calculateBacktestMetrics(item).peakMonth; return month === null ? "N/A" : `${month} bln`; } },
    { key: "verdict", label: "Verdict by Method", width: "w-[145px]", group: "VERDICT", render: (item) => <VerdictBadge value={item.verdict} /> }, { key: "verdictMos", label: "Verdict MoS Main", width: "w-[145px]", group: "VERDICT", render: (item) => <VerdictBadge value={item.verdictMos} /> },
  ];
...
function DetailTableV2({ cases, prices, loadingIds, onSelect, onPriceChange }: { cases: BacktestCase[]; prices: Record<string, number>; loadingIds: Set<string>; onSelect: (testCase: BacktestCase) => void; onPriceChange: (id: string, value: number) => void }) {
  ...
  const columns: DetailColumn[] = [
    ...
    { key: "methods", label: "Undervalued Methods", width: "w-[145px]", group: "METHOD", render: (item) => undervaluedMethods(item, valueFor(item)) },
    ...
    { key: "ret3m", label: "Ret 3M", width: "w-[190px]", group: "RETURNS", render: (item) => rangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[0]) },
    { key: "ret6m", label: "Ret 6M", width: "w-[190px]", group: "RETURNS", render: (item) => rangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[1]) },
    { key: "ret9m", label: "Ret 9M", width: "w-[190px]", group: "RETURNS", render: (item) => rangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[2]) },
    { key: "ret12m", label: "Ret 12M", width: "w-[200px]", group: "RETURNS", render: (item) => rangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[3]) },
    { key: "price3m", label: "Price 3M", width: "w-[190px]", group: "PRICES", render: (item) => priceRangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[0]) },
    { key: "price6m", label: "Price 6M", width: "w-[190px]", group: "PRICES", render: (item) => priceRangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[1]) },
    { key: "price9m", label: "Price 9M", width: "w-[190px]", group: "PRICES", render: (item) => priceRangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[2]) },
    { key: "price12m", label: "Price 12M", width: "w-[200px]", group: "PRICES", render: (item) => priceRangeText(calculateBacktestMetrics(item, valueFor(item)).ranges[3]) },
    { key: "peakReturn", label: "Ret Peak", width: "w-[77px]", group: "PEAK / TROUGH", render: (item) => percent(calculateBacktestMetrics(item, valueFor(item)).peakReturn) },
    { key: "peakPrice", label: "Harga Peak", width: "w-[84px]", group: "PEAK / TROUGH", render: (item) => price(calculateBacktestMetrics(item, valueFor(item)).peakPrice) },
    { key: "peakMonth", label: "Bln Peak", width: "w-[100px]", group: "PEAK / TROUGH", render: (item) => { const month = calculateBacktestMetrics(item, valueFor(item)).peakMonth; return month === null ? "N/A" : `${month} bln`; } },
    { key: "troughReturn", label: "Ret Trough", width: "w-[84px]", group: "PEAK / TROUGH", render: (item) => percent(calculateBacktestMetrics(item, valueFor(item)).troughReturn) },
    { key: "downPrice", label: "Harga Trough", width: "w-[95px]", group: "PEAK / TROUGH", render: (item) => price(calculateBacktestMetrics(item, valueFor(item)).downPrice) },
    { key: "troughMonth", label: "Bln Trough", width: "w-[110px]", group: "PEAK / TROUGH", render: (item) => { const month = calculateBacktestMetrics(item, valueFor(item)).troughMonth; return month === null ? "N/A" : `${month} bln`; } },
...
export function BacktestTabContent({ stock }: { stock: StockDetail }) {
  ...
  const overviewAggregates = useMemo(
    () => aggregateBacktestOverview(cases, analysisPrices),
    [cases, analysisPrices],
  );
